...
def parse_all_images(filename, page, pagenum, text_blocks):
    ...
    for image_info in image_info_list:
        ...
        if (
            img_bbox.width < page_rect.width / 20
            or img_bbox.height < page_rect.height / 20
        ):
            continue
...
def load_multimodal_data(files):
    ...
    for file in files:
        ...
        if file_extension in (".png", ".jpg", ".jpeg"):
            ...
            doc = Document(
                text=image_text, metadata={"source": file.name, "type": "image"}
            )
...
def load_data_from_directory(directory):
    ...
    for filename in os.listdir(directory):
        ...
        if file_extension in (".png", ".jpg", ".jpeg"):
            ...
            image_text = describe_image(image_content)
            doc = Document(
                text=image_text, metadata={"source": filename, "type": "image"}
            )
            print(doc)
            documents.append(doc)
        elif file_extension == ".pdf":
